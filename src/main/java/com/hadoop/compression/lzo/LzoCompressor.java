...
public class LzoCompressor implements Compressor {
  ...
  public static final int LZO_LIBRARY_VERSION = 
    LzoVersion.LZO_LIBRARY_VERSION;
  ...
  public LzoCompressor(CompressionStrategy strategy, int outputBufferSize) {
    ...
    this.outputBuffer = new byte[outputBufferSize + (outputBufferSize >> 3) + 
                                 256];
    reset();
  }
  ...
  private boolean testInvariants() {
    if (inputHoldoverBuffer != null) {
      if (inputBufferLen != 0 && inputBufferLen != inputBuffer.length)
        throw new IllegalStateException("Funny input buffer length " + 
                                        inputBufferLen + " with array size " +
                                        inputBuffer.length + " and holdover.");
      if (inputHoldoverBufferPos < 0)
        throw new IllegalStateException("Using holdover buffer, but invalid " +
                                        "holdover position " + 
      ...
      if (inputHoldoverBufferPos != -1)
        throw new IllegalStateException("No holdover buffer, but valid " +
                                        "holdover position " + 
                                        inputHoldoverBufferPos);
    ...
    if (outputBufferLen.value < 0)
      throw new IllegalStateException("Output buffer overrun pos=" + 
                                      outputBufferPos + "; len=" + 
                                      outputBufferLen);
  ...
  public void setInput(byte[] b, int off, int len) {
    ...
    if (off < 0 || len < 0 || off > b.length - len)
      throw new ArrayIndexOutOfBoundsException("Illegal range in buffer: " +
                                               ...
                                               ", length=" + len);
    if (inputHoldoverBuffer != null)
      throw new IllegalStateException("Cannot accept input while holdover " +
                                      "is present.");
    ...
    // Unfortunately, we have to do this here. This is so, so, so wrong.
    inputByteCount += len;
  ...
  public boolean finished() {
    ...
    return finished && outputBufferLen.value == 0 && inputBufferLen == 0 && 
  ...
  private void compact() {
    ...
    if (inputHoldoverBufferLen <= remaining) {
      ...
      System.arraycopy(inputHoldoverBuffer, inputHoldoverBufferPos, 
                       inputBuffer, inputBufferLen, inputHoldoverBufferLen);
      ...
      System.arraycopy(inputHoldoverBuffer, inputHoldoverBufferPos, 
                       inputBuffer, inputBufferLen, remaining);
  ...
  @Override
    ...
    if (outputBufferLen.value == 0) {
      ...
      if (inputBufferLen > 0) {
        ...
        // If this is ever less than inputBuffer.length, then we
        // should have copied it into the input buffer.
        compressBufferLen = Math.min(inputBuffer.length, 
                                     inputHoldoverBufferLen);
        assert compressBufferLen == inputBuffer.length : 
          "Compressing less than one block of holdover.";
      ...
      try {
        int code = compressor.compress(compressBuffer, compressBufferPos, 
                                       compressBufferLen, outputBuffer, 
