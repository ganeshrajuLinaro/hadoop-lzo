...
public class LzopInputStream extends LzoInputStream {
  ...
  public void resetState() throws IOException {
    ...
    this.c_crc32_c = ((flags & LzopConstants.F_CRC32_C) == 0) ?
        null :
        new CRC32();
    this.c_crc32_d = ((flags & LzopConstants.F_CRC32_D) == 0) ?
        null :
        new CRC32();
    this.c_adler32_c = ((flags & LzopConstants.F_ADLER32_C) == 0) ?
        null :
        new Adler32();
   ...
   */
  private int readHeaderItem(byte[] buf, int len, Adler32 adler, CRC32 crc32
                             ) throws IOException {
  ...
  private void testChecksum(Checksum csum, int value, byte[] data, int off, int len) throws IOException {
    ...
    if (value != (int) csum.getValue()) {
      throw new IOException(
          "Checksum failure: " + "Expected " + Integer.toHexString(value) +
  ...
  protected boolean readBlock() throws IOException {
    ...
    if (outputBufferLength == inputBufferLength) {
      ...
      testChecksum(c_adler32_d, v_adler32_d, outputBuffer, 0, 
                   outputBufferLength);
