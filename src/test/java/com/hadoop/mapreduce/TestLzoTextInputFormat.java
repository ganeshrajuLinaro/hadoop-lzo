...
public class TestLzoTextInputFormat extends TestCase {
   ...
   * figure out the input splits, so to fake it out, I need to use a
   * custom file system that generates the right BlockLocations based on the
   * block size.
   */
  private static class TestFileSystem extends FilterFileSystem {
    TestFileSystem() throws IOException {
      super(FileSystem.getLocal(new Configuration()));
    }
