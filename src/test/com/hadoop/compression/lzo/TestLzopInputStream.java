...
/**
 * Test the LzopInputStream, making sure we get the same bytes when
 * reading a compressed file through it as when we read the
 * corresponding uncompressed file through a standard input stream.
 */
public class TestLzopInputStream extends TestCase {
  ...
  NoSuchAlgorithmException, InterruptedException {
    ...
    File lzoFile = new File(inputDataPath, filename + 
    ...
    LzopInputStream lzoIn = 
      new LzopInputStream(new FileInputStream(lzoFile.getAbsolutePath()), 
    ...
    while ((textLine = textBr.readLine()) != null) {
      ...
      if (!lzoLine.equals(textLine)) {
        LOG.error("LZO decoding mismatch on line " + line + " of file " + 
		  filename);
        LOG.error("Text line: [" + textLine + "], which has length " + 
		  textLine.length());
